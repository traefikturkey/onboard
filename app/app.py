...
def load_file(file_name):
    ...
    file_path = os.path.join(current_working_directory, 'configs', file_name)  # Adjust file path for the configs subdirectory
...
def index():
    ...
    for feed in feeds:
        ...
        columns[column_index].append({'title': feed['name'], 'link': feed['link'], 'url': feed['url'], 'summary_enabled': bool(feed.get('summary', True))})
        ...
        for item in column:
            if 'url' in item:  # If it's a bookmark or a feed with a URL
                ...
                if not parsed_item:
                    ...
                    parsed_item = {
                        'title': item['title'],
                        'articles': [{'title': clean_html(entry.get('title', 'No Title')), 'link': entry.link, 'summary': clean_html(entry.get('summary', ''), )} for entry in parsed_feed.entries[:10]] if 'entries' in parsed_feed else []
                    }
