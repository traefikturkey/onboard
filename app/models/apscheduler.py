...
try:
  # APScheduler is optional for test environments. Import lazily and
  # fall back gracefully if not available to avoid import-time failures
  # when running tests that don't require the scheduler.
...
_BG_SCHEDULER: Optional[Any] = None


def _create_background_scheduler() -> Optional[Any]:
  """Create a BackgroundScheduler instance or return None if APScheduler
  isn't installed in the running environment.
  """
...
class Scheduler:
  ...
  def shutdown():
    ...
    try:
      ...
      # Swallow shutdown errors to preserve graceful behavior in tests
      pass

  @staticmethod
  def clear_jobs():
    """Remove all scheduled jobs if a scheduler exists.

    This is defensive: during tests or in minimal environments the
    underlying BackgroundScheduler may not be present, so this becomes
    a no-op instead of raising an AttributeError on None.
    """
    sched = Scheduler.getScheduler()
    if not sched:
      return
    try:
      ...
      # Ignore failures when clearing jobs in constrained/test envs
      pass
  ...
  def getScheduler() -> Optional[Any]:
    ...
    if _BG_SCHEDULER is None:
      ...
      if (
        ...
        if _BG_SCHEDULER is None:
          # Can't start a scheduler we couldn't create
        ...
        if is_dev_mode and not is_main_process:
          ...
          if _BG_SCHEDULER and not getattr(_BG_SCHEDULER, "running", False):
            Scheduler.start()
        elif _BG_SCHEDULER and not getattr(_BG_SCHEDULER, "running", False):
          Scheduler.start()
