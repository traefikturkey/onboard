...
def test_init_sets_id_and_template(widget_data):
  with patch("app.models.widget.calculate_sha1_hash", return_value="hashed_id") as mock_hash:
...
def test_from_dict_dispatch(widget_data):
  ...
  with patch("app.models.feed.Feed", MagicMock(return_value="feed_instance")) as mock_feed:
  ...
  with patch("app.models.bookmarks.Bookmarks", MagicMock(return_value="bookmarks_instance")) as mock_bookmarks:
    result = Widget.from_dict(widget_data)
  ...
  with patch("app.models.iframe.Iframe", MagicMock(return_value="iframe_instance")) as mock_iframe:
    result = Widget.from_dict(widget_data)
