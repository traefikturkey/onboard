...
def test_from_dict_with_columns(row_dict_with_columns):
    ...
    with patch(
    ...
    ) as mock_column_from_dict, patch(
        "app.models.row.from_list", return_value=[mock_column, mock_column]
    ) as mock_from_list:
        ...
        mock_from_list.assert_called_once_with(
            mock_column_from_dict, row_dict_with_columns["columns"]
        )
...
def test_from_dict_with_widgets(row_dict_with_widgets):
    ...
    with patch(
    ...
    ) as mock_widget_from_dict, patch(
        "app.models.row.from_list", return_value=[mock_widget, mock_widget, mock_widget]
    ) as mock_from_list, patch(
        ...
        mock_from_list.assert_called_once_with(
            mock_widget_from_dict, row_dict_with_widgets["widgets"]
        )
