...
class TestIframe(unittest.TestCase):
  def test_iframe_sets_src(self):
    ...
    # Widget base expects dict with id etc; use minimal
...
class TestFeedArticle(unittest.TestCase):
  def test_title_selection_and_summary(self):
    ...
    # when title empty, title falls back to name/original
    self.assertIn(a.title, ("Orig", "Desc"))

  def test_filters_remove_and_strip(self):
    # prepare parent with filters
    parent = MagicMock()
    parent.filters = [
...
class TestFeed(unittest.TestCase):
  def setUp(self):
    # Ensure WORKING_STORAGE points to a temp dir
  ...
  def test_feed_url_setter_sets_id(self):
    ...
    f = Feed(widget)
    # id should be set from feed_url
    self.assertIsNotNone(f.id)
    self.assertTrue(len(f.id) > 0)
  ...
  def test_processors_with_missing_processor_uses_noop(self):
    widget = self.make_widget()
    f = Feed(widget)
    # create a processor entry referencing non-existing file
  ...
  def test_remove_duplicate_articles_keeps_processed(self):
    ...
    # should keep one for id=1 (the processed one) and one for id=2
  ...
  def test_save_articles_writes_cache(self):
    ...
    articles = []
    res = f.save_articles(articles)
    # after saving, cache file should exist
