...
class TestFaviconUtilsAndLinkTracker(unittest.TestCase):
    ...
    def test_normalize_and_base_and_filenames(self):
        self.assertEqual(
            favicon_utils.normalize_domain("http://example.com/path"), "example.com"
        )
        self.assertEqual(
            favicon_utils.base("http://example.com/path"), "http://example.com"
        )
        self.assertEqual(
            favicon_utils.favicon_filename("http://example.com"),
            "example.com.favicon.ico",
        )
        self.assertEqual(
            favicon_utils.favicon_failed_filename("http://example.com"),
            "example.com.failed",
        )
    ...
    def test_favicon_path_and_download_failure_creates_failed_file(self):
        ...
        with patch("app.services.favicon_utils.make_request", return_value=R()):
            favicon_utils._download(
                "http://example.com", str(icon_dir), "http://example.com/favicon.ico"
            )
