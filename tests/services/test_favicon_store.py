...
def test_icon_path_and_none(tmp_path, monkeypatch):
  ...
  monkeypatch.setattr("app.services.favicon_store.favicon_filename", lambda u: "ic.ico")
...
def test_fetch_favicons_from_schedules_dedupe(tmp_path, monkeypatch):
  ...
  monkeypatch.setattr("app.services.favicon_store.Scheduler.getScheduler", lambda: fake_scheduler)
  ...
  assert any(call["kw"].get("id") == "fetch_favicons" for call in fake_scheduler.calls)

  # inspect the call and ensure deduped bases are passed
  fetch_call = next(call for call in fake_scheduler.calls if call["kw"].get("id") == "fetch_favicons")
