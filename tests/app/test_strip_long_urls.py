...
def test_short_url_kept():
    ...
    assert out[0].title == "Short link http://ex.com/abc"
...
def test_long_url_stripped():
    ...
    out = processor.process([a])
    assert "http" not in out[0].title
    assert out[0].title.strip() == "Breaking"
...
def test_mixed_urls_only_long_removed():
    ...
    out = processor.process([a])
    assert "http://t.co/abc" in out[0].title
    assert "example.com" not in out[0].title
...
def test_only_long_url_keeps_original():
    ...
    out = processor.process([a])
    # Processor is conservative: if stripping produces empty title, original is preserved
    assert out[0].title == long_url
