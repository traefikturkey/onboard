...
def step_impl_then_see_webpage(context):
  assert context.response is not None, f"No response received: {getattr(context, 'error', '')}"
  assert context.response.status_code == 200, f"Unexpected status code: {getattr(context.response, 'status_code', None)}"
  assert '<html' in context.response.text.lower(), "Response does not contain HTML content"
