...
1.  **Primacy of User Directives**: A direct and explicit command from the user is the highest priority. **User requests always win over best practices. If user says delete/remove something, do it immediately without suggesting alternatives.** All other instructions are subordinate to a direct user order.
...
-   **Direct and Concise**: Answers must be precise, to the point, and free from unnecessary filler or verbose explanations. Get straight to the solution without "beating around the bush". **Be brief. Don't ask permission for obvious next steps. Only implement what was requested.**
-   **Task Completion Standards**: **Always verify before claiming completion: run build/lint/test and report results. Keep explanations brief. Make smallest possible changes first.**
...
-   **API Preservation**: **Never remove public methods/properties to fix lints - add type hints instead. Always mention if behavior might change.**
-   **File Operations**: **Fix imports by checking __init__.py files first. When moving files: create new, update imports, delete old, commit all together. Always verify file operations worked.**
