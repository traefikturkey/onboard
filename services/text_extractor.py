...
class TextExtractor:
    ...
    def _extract_readability(self, html: str) -> tuple[str, str]:
        """Return (title, text). Fallbacks to lightweight methods if readability is unavailable."""
        try:
            ...
            # crude text extraction without heavy deps
            text = _strip_html(content_html)
            return title, text
        except Exception:
            # fallback: parse <title> and basic text
...
def _strip_html(html: str) -> str:
    # minimal HTML text stripper without external libs
    import re
